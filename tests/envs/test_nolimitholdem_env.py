...
class TestNolimitholdemEnv(unittest.TestCase):
    ...
    def test_reset_and_extract_state(self):
        env = rlcard_fork.make('no-limit-holdem')
        state, _ = env.reset()
    ...
    def test_step_back(self):
        env = rlcard_fork.make('no-limit-holdem', config={'allow_step_back':True})
    ...
    def test_multiplayers(self):
        env = rlcard_fork.make('no-limit-holdem', config={'game_num_players':5})
        num_players = env.game.get_num_players()
        self.assertEqual(num_players, 5)

    def test_config_chips(self):
        env = rlcard_fork.make('no-limit-holdem', config={'game_num_players':5, 'chips_for_each':100})
        env.game.init_game()
